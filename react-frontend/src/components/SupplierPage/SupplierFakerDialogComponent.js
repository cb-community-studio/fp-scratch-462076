...
import supplierFakerFactory from "./supplierFakerFactory";
...
const SupplierFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = supplierFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("supplier").create(elem));
...
export default connect(null, mapDispatch)(SupplierFakerDialogComponent);
