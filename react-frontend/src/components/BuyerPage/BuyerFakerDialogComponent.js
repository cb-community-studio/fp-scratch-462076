...
import buyerFakerFactory from "./buyerFakerFactory";
...
const BuyerFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = buyerFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("buyer").create(elem));
...
export default connect(null, mapDispatch)(BuyerFakerDialogComponent);
